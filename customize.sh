...
MyPrint "║   - 3.模块自定义路径: /sdcard/Android/start_alipay/"
MyPrint "║ "
...
MyPrint " "
#文件夹类型
start_alipay_list_path="/sdcard/Android/start_alipay"
start_alipay_list_path_old="/sdcard/Android/start_alipay_old"
cron_set_dir="${start_alipay_list_path}/定时任务"

#文件类型
#Black_List="${start_alipay_list_path}/黑名单.prop"
White_List="${start_alipay_list_path}/勿扰名单.prop"
...
# 判断是否安装过
if [[ -d ${script_dir}/tmp/DATE ]] && [[ -d ${start_alipay_list_path} ]]; then
  mkdir -p "$start_alipay_list_path_old/定时任务"
  #cp -f "$Black_List" "$start_alipay_list_path_old"
  cp -f "$White_List" "$start_alipay_list_path_old"
  cp -rf "$cron_set_dir" "$start_alipay_list_path_old"
  #rm -rf "$Black_List"
  rm -rf "$White_List"
  rm -rf "$cron_set_dir"
...
#[[ -f ${Black_List} ]] || cp -r "${MODPATH}"/AndroidFile/黑名单.prop ${start_alipay_list_path}/
[[ -f ${White_List} ]] || cp -r "${MODPATH}"/AndroidFile/勿扰名单.prop ${start_alipay_list_path}/
