...
if [[ -f "$MODDIR"/script/set_cron.d/root ]]; then
  ...
  echo "*/1 * * * * $MODDIR/script/Run_clear.sh" > "$MODDIR"/script/cron.d/root
...
if [[ $(pgrep -f "crond_clear_the_blacklist/script/cron.d" | grep -vc grep) -ge 1 ]]; then
  basic_Information
  logd "$(cat "$MODDIR"/print_set)"
  logd "开始运行: [$crond_root_file]"
  logd "------------------------------------------------------------"
elif [[ $(pgrep -f "crond_clear_the_blacklist/script/set_cron.d" | grep -vc grep) -ge 1 ]]; then
  basic_Information
  logd "$(cat "$MODDIR"/print_set)"
  logd "开始运行: [$crond_root_file]"
  logd "------------------------------------------------------------"
